...
public class Aufgabe2u3 extends Application {
	...
	public void start(Stage prim) throws Exception {
		...
		EventHandler<MouseEvent> mouseHandler = new EventHandler<MouseEvent>() {
			public void handle(MouseEvent event) {
				...
				if (event.getEventType() == MouseEvent.MOUSE_CLICKED) {
					...
					((PianoButton) event.getTarget()).startFullDrag();
				}
			}
		};

		EventHandler<MouseDragEvent> mouseDragHandler = new EventHandler<MouseDragEvent>() {
		...
		for (int i = 0; i < 24; i++) {
			...
			pb.addEventHandler(MouseDragEvent.ANY, mouseDragHandler);
		}
